...
import { getCurrentUser, signOut, updateCurrentUser, type AuthUser } from './lib/auth';
...
function ProtectedLayout({ children }: { children: ReactNode }) {
  ...
  return (
    <div className="flex min-h-screen bg-slate-50">
      <Sidebar />
      <main key={`${user.id}-${user.departmentId}-${user.fullName}`} className="min-w-0 flex-1 overflow-y-auto pt-[73px] lg:pt-0 lg:ml-0">
        {children}
      </main>
    </div>
  );
